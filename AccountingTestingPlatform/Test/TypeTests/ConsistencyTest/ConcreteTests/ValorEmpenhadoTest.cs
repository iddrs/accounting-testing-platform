...
{
    class ValorEmpenhadoTest : ConsistencyTestBase
    {

        public ValorEmpenhadoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5229201%';",
            ];
            string[] sql2 = [
                $"select sum(valor_empenhado)::decimal from pad.bal_desp where remessa = {_remessa} and entidade like '{entidade}';",
            ];
