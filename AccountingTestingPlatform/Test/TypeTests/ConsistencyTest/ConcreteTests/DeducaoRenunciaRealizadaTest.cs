...
{
    class DeducaoRenunciaRealizadaTest : ConsistencyTestBase
    {

        public DeducaoRenunciaRealizadaTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual *-1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '62132%';",
            ];
            string[] sql2 = [
                $"select sum(receita_realizada *-1)::decimal from pad.bal_rec where remessa = {_remessa} and entidade like '{entidade}' and tipo_nivel_receita like 'A' and caracteristica_peculiar_receita in (101, 103);",
            ];
            ...
            return ("Dedução por renúncia de receita realizada", result);
        }
