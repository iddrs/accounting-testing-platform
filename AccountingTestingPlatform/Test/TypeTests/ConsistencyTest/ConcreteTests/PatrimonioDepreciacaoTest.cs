...
{
    class PatrimonioDepreciacaoTest : ConsistencyTestBase
    {

        public PatrimonioDepreciacaoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        protected override (string, ITestResult) ExecuteTest(string entidade)
        {
            ...
            string[] sql1 = [
                $"select sum(valor)::decimal from auxiliar.dados_manuais where remessa = {_remessa} and entidade like '{entidade}' and item like 'Patrimônio - depreciação do sistema';",
            ];
            string[] sql2 = [
                $"select sum(valor_lancamento)::decimal from pad.tce_4111 where remessa = {_remessa} and entidade like '{entidade}' and data_lancamento between '{date.ToString("yyyy-MM-dd")}' and '{datas.Item2}' and conta_contabil like '1238%' and tipo_lancamento like 'C';",
            ];
            ...
            return ("Patrimônio: Depreciação", result);
        }
