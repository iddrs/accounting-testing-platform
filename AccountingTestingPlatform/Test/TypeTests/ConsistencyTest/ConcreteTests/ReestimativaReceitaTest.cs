...
{
    class ReestimativaReceitaTest : ConsistencyTestBase
    {

        public ReestimativaReceitaTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5212101%';",
            ];
            string[] sql2 = [
                $"select sum(previsao_atualizada)::decimal from pad.bal_rec where remessa = {_remessa} and entidade like '{entidade}' and tipo_nivel_receita like 'A';",
                $"select sum(receita_orcada *-1)::decimal from pad.bal_rec where remessa = {_remessa} and entidade like '{entidade}' and tipo_nivel_receita like 'A';",
            ];
            ...
            return ("Reestimativa da receita", result);
        }
