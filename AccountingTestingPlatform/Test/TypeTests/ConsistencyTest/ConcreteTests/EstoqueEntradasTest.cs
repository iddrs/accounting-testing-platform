...
{
    class EstoqueEntradasTest : ConsistencyTestBase
    {

        public EstoqueEntradasTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        protected override (string, ITestResult) ExecuteTest(string entidade)
        {
            ...
            string[] sql1 = [
                $"select sum(valor)::decimal from auxiliar.dados_manuais where remessa = {_remessa} and entidade like '{entidade}' and item like 'Estoque - entradas';",
            ];
            string[] sql2 = [
                $"select sum(valor_lancamento)::decimal from pad.tce_4111 where remessa = {_remessa} and entidade like '{entidade}' and data_lancamento between '{date.ToString("yyyy-MM-dd")}' and '{datas.Item2}' and conta_contabil like '1156199%' and tipo_lancamento like 'D';",
            ];
            ...
            return ("Estoques: entradas", result);
        }
