...
{
    class CreditoSuplementarAbertoTest : ConsistencyTestBase
    {

        public CreditoSuplementarAbertoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5221201%';",
            ];
            string[] sql2 = [
                $"select sum(valor_credito_adicional)::decimal from pad.decreto where remessa = {_remessa} and entidade like '{entidade}' and tipo_credito_adicional = 1;",
            ];
            ...
            return ("Crédito suplementar aberto", result);
        }
