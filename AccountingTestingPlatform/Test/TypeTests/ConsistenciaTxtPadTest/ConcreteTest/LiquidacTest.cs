...
{
    class LiquidacTest : ConsistenciaTxtPadTestBase
    {

        public LiquidacTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        protected override (string, ITestResult) ExecuteTest(string entidade)
        {
            ...
            {
                {"Liquidado", $"select sum(valor_liquidacao)::decimal from pad.liquidac where remessa = {_remessa} and entidade like '{entidade}' and ano_empenho = {_remessa.Substring(0, 4)};" },
