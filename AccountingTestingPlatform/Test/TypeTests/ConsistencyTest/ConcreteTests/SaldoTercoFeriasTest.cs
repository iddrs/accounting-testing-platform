...
{
    class SaldoTercoFeriasTest : ConsistencyTestBase
    {

        public SaldoTercoFeriasTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '113110104%';",
            ];
            string[] sql2 = [
                $"select 0;",
            ];
            ...
            return ("Saldo da conta de 1/3 de férias", result);
        }
