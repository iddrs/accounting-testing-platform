...
{
    class EstoqueSaldoInicialTest : ConsistencyTestBase
    {

        public EstoqueSaldoInicialTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        protected override (string, ITestResult) ExecuteTest(string entidade)
        {
            ...
            string[] sql1 = [
                $"select sum(valor)::decimal from auxiliar.dados_manuais where remessa = {_remessa} and entidade like '{entidade}' and item like 'Estoque - saldo inicial';",
            ];
            string[] sql2 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {remessaAnterior} and entidade like '{entidade}' and conta_contabil like '1156199%' and escrituracao like 'S';",
            ];
            ...
            return ("Estoques: saldo inicial", result);
        }
