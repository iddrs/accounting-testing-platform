...
{
    class RateioCofronTest : ConsistencyTestBase
    {

        public RateioCofronTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '8531005%';",
            ];
            string[] sql2 = [
                $"select sum(valor_pagamento)::decimal from pad.pagamento where remessa = {_remessa} and entidade like '{entidade}' and credor = 451 and rubrica like '__7170%' and ano_empenho = {GetAnoRemessa(_remessa)} and data_pagamento between '{GetLimitesRemessa(_remessa).Item1}' and '{GetLimitesRemessa(_remessa).Item2}';",
            ];
            ...
            return ("Despesas de contrato de rateio com o Cofron", result);
        }
