...
{
    class PrevisaoInicialReceitaBrutaTest : ConsistencyTestBase
    {

        public PrevisaoInicialReceitaBrutaTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '52111%';",
            ];
            string[] sql2 = [
                $"select sum(receita_orcada)::decimal from pad.bal_rec where remessa = {_remessa} and entidade like '{entidade}' and tipo_nivel_receita like 'A' and categoria_receita not like 'dedutora';",
            ];
            ...
            return ("Previsão inicial da receita bruta", result);
        }
