...
{
    class RppInscricaoNoExercicioTest : EncerramentoAnualTestBase
    {

        public RppInscricaoNoExercicioTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bver_enc where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5327%';",
            ];
            string[] sql2 = [
            ...
            return ("Restos a pagar processados - inscrição no exercício", result);
        }
