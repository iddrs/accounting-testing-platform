...
{
    class PatrimonioImobilizadoTest : ConsistencyTestBase
    {

        public PatrimonioImobilizadoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        protected override (string, ITestResult) ExecuteTest(string entidade)
        {
            ...
            string[] sql1 = [
                $"select sum(valor)::decimal from auxiliar.dados_manuais where remessa = {_remessa} and entidade like '{entidade}' and item like 'Patrimônio - saldo atual do sistema';",
            ];
            string[] sql2 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and conta_contabil like '123%' and escrituracao like 'S';",
                $"select sum(saldo_atual *-1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and conta_contabil like '123119908%' and escrituracao like 'S';",
                $"select sum(saldo_atual *-1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and conta_contabil like '123119999%' and escrituracao like 'S';",
                $"select sum(saldo_atual *-1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and conta_contabil like '123210501%' and escrituracao like 'S';",
            ...
            return ("Patrimônio x Saldo do Imobilizado", result);
        }
