...
{
    class PrevisaoInicialDeducaoFundebTest : ConsistencyTestBase
    {

        public PrevisaoInicialDeducaoFundebTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual * -1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '521120101%';",
            ];
            string[] sql2 = [
                $"select sum(receita_orcada * -1)::decimal from pad.bal_rec where remessa = {_remessa} and entidade like '{entidade}' and tipo_nivel_receita like 'A' and caracteristica_peculiar_receita in (105);",
            ];
            ...
            return ("Previsão inicial da dedução para o Fundeb", result);
        }
