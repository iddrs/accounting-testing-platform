...
{
    class PrevisaoInicialDeducaoRenunciaTest : ConsistencyTestBase
    {

        public PrevisaoInicialDeducaoRenunciaTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual * -1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5211202%';",
            ];
            string[] sql2 = [
                $"select sum(receita_orcada * -1)::decimal from pad.bal_rec where remessa = {_remessa} and entidade like '{entidade}' and tipo_nivel_receita like 'A' and caracteristica_peculiar_receita in (101, 103);",
            ];
            ...
            return ("Previsão inicial da dedução por renúncia", result);
        }
