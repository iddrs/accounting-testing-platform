...
{
    class CreditoEspecialAbertoTest : ConsistencyTestBase
    {

        public CreditoEspecialAbertoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '522120201%';",
            ];
            string[] sql2 = [
                $"select sum(valor_credito_adicional)::decimal from pad.decreto where remessa = {_remessa} and entidade like '{entidade}' and tipo_credito_adicional = 2 and data_reabertura_credito_adicional is null;",
            ];
            ...
            return ("Crédito especial aberto", result);
        }
