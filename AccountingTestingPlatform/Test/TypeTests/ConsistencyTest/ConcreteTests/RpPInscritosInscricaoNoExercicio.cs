...
{
    class RpPInscritosInscricaoNoExercicio : ConsistencyTestBase
    {

        public RpPInscritosInscricaoNoExercicio(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5327%';",
            ];
            string[] sql2 = [
                $"select 0::decimal;",
            ];
