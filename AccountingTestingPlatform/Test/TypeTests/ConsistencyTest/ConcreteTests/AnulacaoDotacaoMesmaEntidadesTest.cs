...
{
    class AnulacaoDotacaoMesmaEntidadesTest : ConsistencyTestBase
    {

        public AnulacaoDotacaoMesmaEntidadesTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(valor_credito_adicional)::decimal from pad.decreto where remessa = {_remessa} and entidade like '{entidade}' and origem_recurso = 5;",
            ];
            string[] sql2 = [
                $"select sum(valor_reducao_dotacao)::decimal from pad.decreto where remessa = {_remessa} and entidade like '{entidade}' and origem_recurso = 5;",
            ];
            ...
            return ("Anulação de dotação na mesma entidade", result);
        }
