...
{
    class RpNpInscritosUltimoExercicioTest : ConsistencyTestBase
    {

        public RpNpInscritosUltimoExercicioTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5311%';",
            ];
            string[] sql2 = [
                $"select sum(nao_processado_inscritos_ultimo_exercicio)::decimal from pad.restos_pagar where remessa = {_remessa} and entidade like '{entidade}';",
            ];
            ...
            return ("Restos a pagar não processados inscritos no último exercício", result);
        }
