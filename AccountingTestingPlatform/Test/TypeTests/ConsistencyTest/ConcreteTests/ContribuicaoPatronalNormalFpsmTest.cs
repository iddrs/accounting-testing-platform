...
{
    class ContribuicaoPatronalNormalFpsmTest : ConsistencyTestBase
    {

        public ContribuicaoPatronalNormalFpsmTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        protected override (string, ITestResult) ExecuteTest(string entidade)
        {
            ...
            string[] sql2 = [
                ...
                $"select sum(valor_liquidacao)::decimal from pad.liquidacao where remessa = {_remessa} and ano_empenho = {GetAnoRemessa(_remessa)} and data_liquidacao between '{GetLimitesRemessa(_remessa).Item1}' and '{GetLimitesRemessa(_remessa).Item2}' and rubrica like '31911310%';",
                $"select sum(valor_pagamento *-1)::decimal from pad.pagamento where remessa = {_remessa} and ano_empenho = {GetAnoRemessa(_remessa)} and data_pagamento between '{GetLimitesRemessa(_remessa).Item1}' and '{GetLimitesRemessa(_remessa).Item2}' and rubrica like '31911308%';",
                $"select sum(valor_pagamento *-1)::decimal from pad.pagamento where remessa = {_remessa} and ano_empenho = {GetAnoRemessa(_remessa)} and data_pagamento between '{GetLimitesRemessa(_remessa).Item1}' and '{GetLimitesRemessa(_remessa).Item2}' and rubrica like '31911310%';",
            ];
            ...
            return ("Contribuição patronal normal a receber pelo FPSM", result);
        }
