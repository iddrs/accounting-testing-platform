...
{
    class ContribuicaoRPPSAPagarTest : ConsistencyTestBase
    {

        public ContribuicaoRPPSAPagarTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '2114201%' and indicador_superavit_financeiro like 'F';",
            ];
            string[] sql2 = [
                $"select sum(liquidado_a_pagar)::decimal from pad.bal_desp where remessa = {_remessa} and entidade like '{entidade}' and elemento like '319113%';",
            ];
            ...
            return ("Contribuição ao RPPS a pagar", result);
        }
