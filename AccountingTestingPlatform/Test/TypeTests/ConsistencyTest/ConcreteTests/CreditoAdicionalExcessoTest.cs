...
{
    class CreditoAdicionalExcessoTest : ConsistencyTestBase
    {

        public CreditoAdicionalExcessoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5221302%';",
            ];
            string[] sql2 = [
                $"select sum(valor_credito_adicional)::decimal from pad.decreto where remessa = {_remessa} and entidade like '{entidade}' and origem_recurso = 2;",
            ];
            ...
            return ("Crédito adicional por excesso de arrecadação", result);
        }
