...
{
    class RpNpInscritosExercicioAnteriorTest : ConsistencyTestBase
    {

        public RpNpInscritosExercicioAnteriorTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5312%';",
            ];
            string[] sql2 = [
                $"select sum(saldo_nao_processado_inscritos_exercicios_anteriores)::decimal from pad.restos_pagar where remessa = {_remessa} and entidade like '{entidade}';",
            ];
            ...
            return ("Restos a pagar não processados inscritos em exercícios anteriores", result);
        }
