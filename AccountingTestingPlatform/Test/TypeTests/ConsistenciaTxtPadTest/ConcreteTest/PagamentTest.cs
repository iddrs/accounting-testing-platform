...
{
    class PagamentTest : ConsistenciaTxtPadTestBase
    {

        public PagamentTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        protected override (string, ITestResult) ExecuteTest(string entidade)
        {
            ...
            {
                {"Pago", $"select sum(valor_pagamento)::decimal from pad.pagament where remessa = {_remessa} and entidade like '{entidade}' and ano_empenho = {_remessa.Substring(0, 4)};" },
