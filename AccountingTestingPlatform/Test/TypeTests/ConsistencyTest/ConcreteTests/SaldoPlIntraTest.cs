...
{
    class SaldoPlIntraTest : ConsistencyTestBase
    {

        public SaldoPlIntraTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and escrituracao like 'S' and conta_contabil like '23712%';",
            ];
            string[] sql2 = [
                $"select 0::decimal;",
            ];
            ...
            return ("Saldo do PL intra-OFSS", result);
        }
