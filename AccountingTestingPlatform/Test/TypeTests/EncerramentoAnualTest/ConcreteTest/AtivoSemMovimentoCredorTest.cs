...
{
    class AtivoSemMovimentoCredorTest : EncerramentoAnualTestBase
    {

        public AtivoSemMovimentoCredorTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(movimento_credor)::decimal from pad.bver_enc where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '1%';",
                $"select sum(movimento_credor *-1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '1%';",
            ];
            string[] sql2 = [
                $"select 0::decimal;",
            ];
            ...
            return ("Ativo sem movimento a crédito no encerramento", result);
        }
