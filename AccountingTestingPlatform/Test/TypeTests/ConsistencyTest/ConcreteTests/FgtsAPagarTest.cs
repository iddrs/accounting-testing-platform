...
{
    class FgtsAPagarTest : ConsistencyTestBase
    {

        public FgtsAPagarTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '2114305%' and indicador_superavit_financeiro like 'F';",
            ];
            string[] sql2 = [
                $"select sum(valor_liquidacao)::decimal from pad.liquidacao where remessa = {_remessa} and entidade like '{entidade}' and rubrica like '31901301%';",
                $"select sum(valor_pagamento * -1)::decimal from pad.pagamento where remessa = {_remessa} and entidade like '{entidade}' and rubrica like '31901301%';",
            ];
