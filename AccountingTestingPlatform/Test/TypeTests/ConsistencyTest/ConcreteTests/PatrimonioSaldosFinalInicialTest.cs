...
{
    class PatrimonioSaldosFinalInicialTest : ConsistencyTestBase
    {

        public PatrimonioSaldosFinalInicialTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string remessaAnterior = GetPreviousRemessa(_remessa);
            string[] sql1 = [
                $"select sum(valor)::decimal from auxiliar.dados_manuais where remessa = {remessaAnterior} and entidade like '{entidade}' and item like 'Patrimônio - saldo atual do sistema';",
            ];
            string[] sql2 = [
                $"select sum(valor)::decimal from auxiliar.dados_manuais where remessa = {_remessa} and entidade like '{entidade}' and item like 'Patrimônio - saldo inicial do sistema';",
            ];
            ...
            return ("Patrimônio: saldo final do mês anterior e saldo inicial do mês atual", result);
        }
