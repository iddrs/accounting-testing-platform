...
{
    class RpNpPagoTest : ConsistencyTestBase
    {

        public RpNpPagoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '6314%';",
            ];
            string[] sql2 = [
            ...
            return ("Restos a pagar não processados pagos", result);
        }
