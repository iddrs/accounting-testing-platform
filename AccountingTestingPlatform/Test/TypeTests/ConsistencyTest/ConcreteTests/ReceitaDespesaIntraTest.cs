...
{
    class ReceitaDespesaIntraTest : ConsistencyTestBase
    {

        public ReceitaDespesaIntraTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(receita_realizada)::decimal from pad.bal_rec where remessa = {_remessa} and tipo_nivel_receita like 'A' and categoria_receita like 'intra';",
            ];
            string[] sql2 = [
                $"select sum(valor_pago)::decimal from pad.bal_desp where remessa = {_remessa} and elemento like '__91%';",
            ];
            ...
            return ("Receita e despesa intra-OFSS", result);
        }
