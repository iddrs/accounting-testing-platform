...
{
    class ValorGlobalDotacaoAdicionalTest : ConsistencyTestBase
    {

        public ValorGlobalDotacaoAdicionalTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual *-1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5221399%';",
            ];
            string[] sql2 = [
                $"select sum(valor_credito_adicional)::decimal from pad.decreto where remessa = {_remessa} and entidade like '{entidade}';",
            ];
            ...
            return ("Valor global da dotação adicional", result);
        }
