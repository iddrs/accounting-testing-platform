...
{
    class ReceitaARealizarTest : ConsistencyTestBase
    {

        public ReceitaARealizarTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '6211%';",
            ];
            string[] sql2 = [
                $"select sum(a_arrecadar_atualizado)::decimal from pad.bal_rec where remessa = {_remessa} and entidade like '{entidade}' and tipo_nivel_receita like 'A';",
            ];
            ...
            return ("Receita a arrecadar", result);
        }
