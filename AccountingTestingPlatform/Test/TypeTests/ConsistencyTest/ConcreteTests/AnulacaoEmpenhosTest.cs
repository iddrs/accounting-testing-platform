...
{
    class AnulacaoEmpenhosTest : ConsistencyTestBase
    {

        public AnulacaoEmpenhosTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual *-1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '522920103%';",
            ];
            string[] sql2 = [
                $"select sum(valor_empenho *-1)::decimal from pad.empenho where remessa = {_remessa} and entidade like '{entidade}' and ano_empenho = {GetAnoRemessa(_remessa)} and data_empenho between '{GetLimitesRemessa(_remessa).Item1}' and '{GetLimitesRemessa(_remessa).Item2}' and valor_empenho::decimal < 0.0;",
            ];
            ...
            return ("Anulação de empenhos", result);
        }
