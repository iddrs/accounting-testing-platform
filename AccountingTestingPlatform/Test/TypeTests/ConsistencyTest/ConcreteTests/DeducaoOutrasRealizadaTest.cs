...
{
    class DeducaoOutrasRealizadaTest : ConsistencyTestBase
    {

        public DeducaoOutrasRealizadaTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual *-1)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '62139%';",
            ];
            string[] sql2 = [
                $"select sum(receita_realizada *-1)::decimal from pad.bal_rec where remessa = {_remessa} and entidade like '{entidade}' and tipo_nivel_receita like 'A' and caracteristica_peculiar_receita in (102, 106, 107, 108, 109);",
            ];
            ...
            return ("Dedução por outros motivos de receita realizada", result);
        }
