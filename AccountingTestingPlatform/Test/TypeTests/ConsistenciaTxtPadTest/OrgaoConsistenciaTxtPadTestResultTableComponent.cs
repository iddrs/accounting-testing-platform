...
class OrgaoConsistenciaTxtPadTestResultTableComponent : IComponent
{
    private readonly DataTable _result;
    public OrgaoConsistenciaTxtPadTestResultTableComponent(DataTable result)
    { 
        _result = result;
    }
    public void Compose(IContainer container)
    {
        container.Table(table =>
        {
            ...
            foreach (DataRow row in _result.Rows)
            {
                ...
                table.Cell().Row(r).Column(2).Element(CellStyle).AlignRight().Text(decimal.Parse(row[1].ToString()).ToString("N0"));
                table.Cell().Row(r).Column(3).Element(CellStyle).AlignRight().Text(decimal.Parse(row[2].ToString()).ToString("N0"));
                if (decimal.Parse(row[3].ToString()) == 0m)
                {
                    table.Cell().Row(r).Column(4).Element(DiferencaSuccessCellStyle).AlignRight().Text(decimal.Parse(row[3].ToString()).ToString("N0"));
                }
                else
                {
                    table.Cell().Row(r).Column(4).Element(DiferencaFailCellStyle).AlignRight().Text($"{decimal.Parse(row[3].ToString()).ToString("N0")} ⚠️");
                }
                r++;
            }
