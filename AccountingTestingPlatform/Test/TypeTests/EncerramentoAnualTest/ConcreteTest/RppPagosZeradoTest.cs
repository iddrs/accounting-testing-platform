...
{
    class RppPagosZeradoTest : EncerramentoAnualTestBase
    {

        public RppPagosZeradoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bver_enc where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '6322%';",
            ];
            string[] sql2 = [
                $"select 0::decimal;",
            ];
            ...
            return ("Restos a pagar processados pagos com saldo zerado no encerramento", result);
        }
