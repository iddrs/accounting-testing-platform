...
{
    class Modalidade71NaoConsorcioTest : ConsistencyTestBase
    {

        public Modalidade71NaoConsorcioTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(valor_empenho)::decimal from pad.empenho where remessa = {_remessa} and entidade like '{entidade}' and ano_empenho = {GetAnoRemessa(_remessa)} and data_empenho between '{GetLimitesRemessa(_remessa).Item1}' and '{GetLimitesRemessa(_remessa).Item2}' and credor not in (451, 8283) and rubrica like '__71%';",
            ];
            string[] sql2 = [
                $"select 0::decimal;",
            ];
            ...
            return ("Modalidade 71 em credor que não é consórcio", result);
        }
