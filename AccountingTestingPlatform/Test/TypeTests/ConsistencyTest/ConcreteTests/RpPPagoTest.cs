...
{
    class RpPPagoTest : ConsistencyTestBase
    {

        public RpPPagoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '6322%';",
            ];
            string[] sql2 = [
                $"select sum(processado_pago)::decimal from pad.restos_pagar where remessa = {_remessa} and entidade like '{entidade}';",
            ];
            ...
            return ("Restos a pagar processados pagos", result);
        }
