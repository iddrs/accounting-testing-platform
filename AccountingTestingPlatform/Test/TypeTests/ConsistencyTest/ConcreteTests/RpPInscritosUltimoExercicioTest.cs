...
{
    class RpPInscritosUltimoExercicioTest : ConsistencyTestBase
    {

        public RpPInscritosUltimoExercicioTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '5321%';",
            ];
            string[] sql2 = [
                $"select sum(processado_inscritos_ultimo_exercicio)::decimal from pad.restos_pagar where remessa = {_remessa} and entidade like '{entidade}';",
            ];
            ...
            return ("Restos a pagar processados inscritos no último exercício", result);
        }
