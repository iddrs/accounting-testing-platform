...
{
    class CreditoExtraordinarioReabertoTest : ConsistencyTestBase
    {

        public CreditoExtraordinarioReabertoTest(NpgsqlConnection connection, string remessa, IReport report)
        {
        ...
        {
            string[] sql1 = [
                $"select sum(saldo_atual)::decimal from pad.bal_ver where remessa = {_remessa} and entidade like '{entidade}' and escrituracao like 'S' and conta_contabil like '522120302%';",
            ];
            string[] sql2 = [
                $"select sum(valor_credito_adicional)::decimal from pad.decreto where remessa = {_remessa} and entidade like '{entidade}' and tipo_credito_adicional = 3 and data_reabertura_credito_adicional is not null;",
            ];
            ...
            return ("Crédito extraordinário reaberto", result);
        }
